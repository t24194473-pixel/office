...
function StatusChanger({ caseId, current, onStop }) {
  const [loading, setLoading] = useState(false);

  const change = async (val, e) => {
    e.stopPropagation();
      ...
      {CASE_STATUSES.map(s => (
        <button
          ...
          className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-medium border transition-all
            ${s.value === current
              ? (STATUS_STYLES[s.value] ?? STATUS_STYLES.archived) + ' shadow-sm'
              : 'bg-gray-50 dark:bg-gray-700 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-600 hover:border-gray-400 dark:hover:border-gray-400'
            }`}
        >
          {s.value === current && <CheckCircle2 size={11} className="shrink-0" />}
          {s.label}
        </button>
      ))}
    </div>
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="min-w-0 flex-1">
          <p className="font-bold text-gray-900 dark:text-white text-sm leading-tight truncate" title={item.title}>
            {item.title}
          </p>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{item.type}</p>
        </div>
      ...
      <div className="border-t border-gray-50 dark:border-gray-700/80 pt-3">
        ...
        {showStatus && (
          <StatusChanger caseId={item.id} current={item.status} />
        )}
      </div>
    </div>
  );
      ...
      {loading ? (
        ...
        <>
                  ...
                  {filtered.map(item => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4 max-w-[220px]">
                        ...
                        <p className="mt-0.5 text-gray-400 dark:text-gray-500 text-xs">{item.type}</p>
                      </td>
          ...
          <div className="md:hidden grid grid-cols-1 sm:grid-cols-2 gap-3">
            {filtered.map(item => (
              <CaseCard key={item.id} item={item} onNavigate={id => navigate(`/cases/${id}`)} />
            ))}
          </div>
        </>
      )}
